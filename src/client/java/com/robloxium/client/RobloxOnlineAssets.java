...
final class RobloxOnlineAssets {
    ...
    static Identifier texture(String id) {
        ...
        return TEXTURES.computeIfAbsent(url, RobloxOnlineAssets::downloadTexture).getNow(null);
    }

    private static Identifier registerTexture(String prefix, DynamicTexture texture) {
        Identifier id = Identifier.fromNamespaceAndPath(
                "robloxium",
                prefix + "_" + Integer.toHexString(System.identityHashCode(texture))
        );
        texture.upload();
    ...
    private static CompletableFuture<Identifier> downloadTexture(String url) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                try (InputStream in = response.body()) {
                    image = NativeImage.read(in); // keep original orientation, no flipY
                }
        ...
        }, IO).thenCompose(image -> {
            if (image == null) return CompletableFuture.completedFuture(null);

            CompletableFuture<Identifier> onClient = new CompletableFuture<>();
            Minecraft.getInstance().execute(() -> {
                DynamicTexture texture = null;
                try {
                    texture = new DynamicTexture(() -> url, image);
                    onClient.complete(registerTexture("robloxium_online", texture));
                } catch (Throwable t) {
                    if (texture != null) {
